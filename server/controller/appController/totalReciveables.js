...
const totalReciveables = async (req, res, next) => {
  try {
    ...
    if (entity === "customers") {
      ...
      let totalToPay = totalPaybles > totalPaid ? totalPaybles - totalPaid : 0;
      ...
      invoices.forEach((invoice) => {
        ...
        } else if (dueDate >= thirtyDaysAgo && dueDate < fifteenDaysAgo) {
          categorizedInvoices.last16To30Days += pendingAmount;
        } else if (dueDate >= fortyFiveDaysAgo && dueDate < thirtyDaysAgo) {
      ...
      if (period === "this_fiscal_year") {
        startOfPeriod = new Date(currentYear, 0, 1); // January 1st of the current year
        endOfPeriod = new Date(currentYear, 11, 31); // December 31st of the current year
      } else if (period === "last_fiscal_year") {
        startOfPeriod = new Date(currentYear - 1, 0, 1); // January 1st of the previous year
        endOfPeriod = new Date(currentYear - 1, 11, 31); // December 31st of the previous year
      } else if (period === "this_month") {
        const today = new Date(); // Today's date
        startOfPeriod = new Date(today.getFullYear(), today.getMonth(), 1); // 1st of the current month
        endOfPeriod = new Date(today.getFullYear(), today.getMonth() + 1, 0); // Last day of the current month
      } else if (period === "last_three_month") {
      ...
      const expenses = await ExpenseDataBase.find({
        expenseDate: { $gte: startOfPeriod, $lte: endOfPeriod },
        tenantId: id,
      });
      ...
      const months = Array.from({ length: 12 }, (_, i) => ({
        name: new Date(startOfPeriod.getFullYear(), i).toLocaleString(
          "default",
