...
const addHeader = (
  ...
  doc
    .moveTo(leftMargin, doc.y) // Adjust the Y position if necessary
  ...
  if (entity.toLowerCase() === "invoices") {
    ...
    doc
      .font("Helvetica")
      .text(`${entityPrefix}/${entityDetails?.no}`, 460, detailsY, {
        width: detailWidth,
        align: "left",
      });
    detailsY += 20;
    ...
    doc
      .font("Helvetica")
      .text(`${jsDateIntoDDMMYY(entityDetails.invoiceDate)}`, 460, detailsY, {
        width: detailWidth,
        align: "left",
      });
    detailsY += 20;
    ...
    doc
      .font("Helvetica")
      .text(`${jsDateIntoDDMMYY(entityDetails.dueDate)}`, 460, detailsY, {
        width: detailWidth,
        align: "left",
      });
    ...
    detailsY += 20;
    doc.font("Helvetica-Bold").text(`Quote Date:`, xForRightSide, detailsY, {
      width: detailWidth,
      align: "left",
    });
    doc
    ...
    doc
      .font("Helvetica")
      .text(`${jsDateIntoDDMMYY(entityDetails.purchaseDate)}`, 460, detailsY, {
        width: detailWidth,
        align: "left",
      });
    detailsY += 20;
    ...
    doc
      .font("Helvetica")
      .text(`${jsDateIntoDDMMYY(entityDetails.deliveryDate)}`, 460, detailsY, {
        width: detailWidth,
        align: "left",
      });
    ...
    doc
      .font("Helvetica")
      .text(`${jsDateIntoDDMMYY(entityDetails?.challanDate)}`, 460, detailsY, {
        width: detailWidth,
        align: "left",
      });
    detailsY += 20;
    ...
    doc.font("Helvetica").text(`${entityDetails?.challanType}`, 460, detailsY, {
      width: detailWidth,
      align: "left",
    });
    detailsY += 20;
    let vehicleDetails = `Vehicle No: ${entityDetails?.vehicleNo || ""}`;
    doc.font("Helvetica-Bold").fontSize(9).text(vehicleDetails, 360, detailsY, {
      width: detailWidth,
      align: "left",
    });
  } else if (entity.toLowerCase() === "workorders") {
    const { type, no, startDate, dueDate, incharge, status } = entityDetails;
    ...
    doc
      .font("Helvetica")
      .text(`${jsDateIntoDDMMYY(startDate)}`, 460, detailsY, {
        width: detailWidth,
        align: "left",
      });
    detailsY += 20;
    ...
    doc.font("Helvetica").text(`${jsDateIntoDDMMYY(dueDate)}`, 460, detailsY, {
      width: detailWidth,
      align: "left",
    });
    detailsY += 20;
  ...
  doc
    .moveTo(leftMargin, horizontalLineY) // Starting at x=4, y=horizontalLineY
    .lineTo(pageWidth - leftMargin, horizontalLineY - 2) // Ending at x=pageWidth - 4, y=horizontalLineY
    .stroke(borderColor);
  // Draw a horizontal line after the header details
  doc.moveTo(300, horizontalLineY).lineTo(300, titleBorder).stroke(borderColor);
...
const addItemsTable2 = async (
  ...
  // for (const [index, item] of entityData.items.entries()) {
  //   let buffer = null;
  //   if (item.image) {
  //     buffer = await downloadImage(item.image); // Download image as buffer
  //   }
  //   items.push({
  //     code: item.code,
  //     description: item.description,
  //     image: buffer, // Attach image buffer to the item
  ...
  const headers = getTableHeaders2(entity, preCol);
  // Draw a triangle
  doc.save().moveTo(500, 150).lineTo(100, 250).lineTo(200, 250).fill("#FF3300");
  console.log(headers, items, doc, "===");
  ...
  headers.forEach((col, index) => {
    console.log(col, "===");
    doc.fontSize(8);
    doc.text(col.title, 10 + index * col.width, yPosition, {
      width: col.width,
      align: "left",
    });
    doc.y + 10;
  ...
  //     // items.forEach((row) => {
  //     //   headers.forEach((col, index) => {
  //     //     if (col.property === "image" && row.image) {
  //     //       // Add image handling (assuming the image is a base64 string or buffer)
  //     //       doc.image(row.image, 50 + index * col.width, yPosition, {
  //     //         width: col.width - 10,
  //     //         height: 40,
  //     //       });
  //     //     } else {
  ...
  //     //         50 + index * col.width,
  //     //         yPosition,
  //     //         { width: col.width, align: "left" }
  //     //       );
  //     //     }
  //     //   });
  //     //   yPosition += 50; // Move down for the next row, adjusting height for images if needed
  //     // });
  ...
  //       doc
  //         .fillColor("#000000")
  //         .text(cell.label, rect.x + 10, rect.y + rect.height / 2, {
  ...
  //       console.log("rohit");
  //     },
  //     // Modify the drawRowCell method to handle image rendering
  //     drawRowCell: (cell, rect) => {
  //       doc.fontSize(9).fillColor("#000000").font("Helvetica");

  //       console.log(cell, rect, "values of the cell"); // Check the content of the cell

  //       //   // If the cell is an image (either a buffer or a valid URL), render the image
  //       //   if (Buffer.isBuffer(cell) || isValidImageUrl(cell)) {
  //       //     // Adjust image size (width, height) according to the row's height
  //       //     try {
  //       //       doc.image(cell, rect.x + 5, rect.y + 5, {
  //       //         width: rect.width - 10, // Image width (make it fit within the column)
  //       //         height: rect.height - 10, // Image height (make it fit within the row)
  //       //         align: "center", // Optional: Align the image
  //       //       });
  //       //     } catch (error) {
  //       //       console.error("Error loading image:", error);
  //       //     }
  //       //   } else {
  //       // If it's not an image, render it as text
  //       doc.text(cell, rect.x + 10, rect.y + rect.height / 2, {
  //         width: rect.width - 10, // Adjust width to fit text
  //         height: rect.height,
  //         align: "center", // Center-align text
  //         valign: "center", // Vertically center-align text
  //       });
  //       //   }

  //       // Draw the border around the cell (for both text and image cells)
  //       doc.rect(rect.x, rect.y, rect.width, rect.height).stroke();
  //     },
...
// Helper function to check if the cell contains a valid image URL
...
const getTableHeaders = (entity, preCol = []) => {
  ...
  order.forEach((value) => {
    ...
    if (preColMap[value]) {
      let additionalHeader = toAddColumn.find((col) => col.value === value);
      if (additionalHeader) {
        finalHeaders.push({
...
const detailsForQuotation = (doc, quotationData, organizationData, curY) => {
  ...
  doc
    .moveTo(leftMargin, customerEndY + 5)
  ...
  items?.forEach((item) => {
    ...
    headers.forEach((header) => {
      ...
      switch (header.title.toUpperCase()) {
        case "CODE":
          doc.text(capitalizeFirstLetter(`${item.product?.code}`) || "", x, y, {
            width: header.width,
            align: "left",
          });
          break;
