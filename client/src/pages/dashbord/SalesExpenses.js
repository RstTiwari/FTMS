...
const SalesExpensesDashboard = () => {
  ...
  return (
    <Card bordered={false} style={{ borderRadius: 8, boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', margin:"20px"}}>
      <Row justify="space-between" align="middle" style={{ marginBottom: '1rem' }}>
        <Col>
          <Title level={4}>Sales and Expenses <Text type="secondary">ⓘ</Text></Title>
        </Col>
        <Col>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} sm={6}>
          <div>
            <Text type="secondary" style={{ display: 'block' }}>Total Sales</Text>
            <Title level={5} style={{ color: '#1890ff' }}>{`₹${total?.totalSales}`}</Title>
          </div>
          <div>
            <Text type="secondary" style={{ display: 'block' }}>Total Receipts</Text>
            <Title level={5} style={{ color: '#52c41a' }}>{`₹${total?.totalReceipts}`}</Title>
          </div>
          <div>
            <Text type="secondary" style={{ display: 'block' }}>Total Expenses</Text>
            <Title level={5} style={{ color: '#ff4d4f' }}>{`₹${total?.totalExpenses}`}</Title>
          </div>
        </Col>
      </Row>
